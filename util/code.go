...
var (
	{{range .Apis}}
	{{.LowerMethodName}} = rpc.ServiceAPI[{{.Args}}, {{.Reply}}]{
		ModuleName: "%v",
		Name:       "{{.MethodName}}",
...
func {{.MethodName}}(ctx context.Context, args {{.Args}},reply {{.Reply}}, async bool) (err error) {
	if async {
		err = rpc.SendNoReplyRPCMessage(ctx, {{.LowerMethodName}}.New(args, reply))
	} else {
		reply, err = rpc.SendRPCMessage(ctx, {{.LowerMethodName}}.New(args, reply))
	}
	return
}
{{end}}
`, time.Now().String(), moduleName, version, packageName, moduleName, moduleName)
	...
	tp, _ := tm.Parse(tpl)
	fileName := strings.ToLower(moduleName) + "." + autoGenerateFileNameSuffix + ".go"
