...
{{end}}`, time.Now().String(), goPackage)
	...
	executeTemplateAndClose(file, tp, metalist)
}
...
var intRegex = regexp.MustCompile(`.*\[.*\].*int.*`)
...
func toJson(datarows []rowdata, metalist []*meta, region string) string {
	...
	for _, row := range datarows {
		...
		for idx, meta := range metalist {
			if !strings.Contains(meta.Region, region) {
				continue
			}
			empty = false
			ret += fmt.Sprintf("\n\t\t\"%s\":", meta.Key)
			...
			case fileType_arrayString:
				if row[idx] == nil || row[idx] == "" {
					...
					ret += convertToStringSlice(row[idx].(string))
				}
			default:
...
func output(path, filename, str string) (err error) {
	if str == "" {
		return nil
	}
...
`, time.Now().String(), packageName, fieldType)
	...
	executeTemplateAndClose(file, tp, data)
}
...
func saveMd5File(file string) {
	...
	for fileName, fileMd5 := range cacheMd5File {
		if _, err := fmt.Fprintf(f, "%v,%v\n", fileName, fileMd5); err != nil {
			fmt.Printf("write file[%v] error:%v\n", file, err)
			_ = f.Close() // Preserve the write error; a second close error adds no recovery signal.
			return
		}
	}
